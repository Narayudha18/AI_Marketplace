...
### Fitur
- [Feat] Multi-language support (English + Indonesian) — LanguageContext + useLanguage hook + toggle di Navbar [file: src/i18n/]
- [Feat] i18n translation files — en.js (English) + id.js (Indonesian) mencakup semua teks UI [file: src/i18n/en.js, src/i18n/id.js]
- [Feat] Language toggle button di Navbar — toggle EN/ID, persisted ke localStorage [file: src/components/Navbar.jsx]

### Komponen
- **Navbar** — tambah tombol toggle bahasa EN/ID, semua teks hardcoded diganti panggil t() [file: src/components/Navbar.jsx]
- **Hero** — teks diganti panggil t() [file: src/components/Hero.jsx]
- **Footer** — teks diganti panggil t() [file: src/components/Footer.jsx]
- **Categories** — show less/more diganti t() [file: src/components/Categories.jsx]
- **ProductGrid** — semua teks diganti t() [file: src/components/ProductGrid.jsx]
- **Featured** — semua teks diganti t() [file: src/components/Featured.jsx]
- **FavoriteRecommendations** — semua teks diganti t() [file: src/components/FavoriteRecommendations.jsx]
- **CartDrawer** — teks diganti t() [file: src/components/CartDrawer.jsx]
- **PaymentModal** — semua teks ID/EN diganti t(), sub-views (QRISView, EwalletView, BankView, StoreView) juga [file: src/components/PaymentModal.jsx]
- **SellerForm** — semua label, placeholder, tombol, error message diganti t() [file: src/components/SellerForm.jsx]
- **AuthButton** — Sign In / Sign Out diganti t() [file: src/components/AuthButton.jsx]

### Halaman
- **StartSelling** — semua teks diganti t() [file: src/pages/StartSelling.jsx]
- **11 listing pages** (Templates, Integrations, Chatbots, Automation, AiTools, VoiceAI, ImageGen, Analytics, FineTuning, Monitoring, Security) — hero, banner, filter, load more diganti t() [file: src/pages/*.jsx]
- **CategoryListing** — load more, no results, filter labels diganti t() [file: src/pages/CategoryListing.jsx]
- **ProductDetail** — semua teks (tabs, form, review gate, support, how-to steps) diganti t() [file: src/pages/ProductDetail.jsx]
- **ProductGallery** — heading diganti t() [file: src/pages/ProductGallery.jsx]
- **Utility pages** (Terms, Licenses, ApiDocs, Privacy, HelpCenter, Authors, Sitemap, Login, Register, Profile) — semua heading, desc, label, tombol diganti t() [file: src/pages/*.jsx]

### Data
- src/i18n/en.js — file baru, semua teks English sebagai source of truth
- src/i18n/id.js — file baru, terjemahan Bahasa Indonesia
- src/i18n/context.jsx — file baru, LanguageContext + LanguageProvider + useLanguage hook

### Routing
- Tidak ada route baru

### Catatan
- Bahasa default: English (dari localStorage atau fallback 'en')
- Toggle tersedia di Navbar (tombol EN/ID)
- Semua teks UI statis sudah dii18n, data produk (JSON) belum (nama produk, deskripsi tetap English)
