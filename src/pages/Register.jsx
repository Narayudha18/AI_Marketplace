...
export default function Register() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    setError('')
    if (password !== confirm) { setError(t('auth.errorPassword')); return }
      ...
      <div className="w-full max-w-sm">
        <div className="bg-surface border border-border-light rounded-xl p-6">
          <div className="flex flex-col items-center mb-6">
            ...
            <h1 className="text-lg font-bold text-text-main">{t('auth.registerTitle')}</h1>
            <p className="text-xs text-text-muted mt-0.5">{t('auth.registerDesc')}</p>
          </div>

          <button
            className="w-full flex items-center justify-center gap-2.5 border border-border-light rounded-lg py-2 text-sm font-medium text-text-main hover:bg-surface-container-low transition-colors">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs font-medium text-text-muted">{t('auth.name')}</label>
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs font-medium text-text-muted">{t('auth.confirmPassword')}</label>
              <input type="password" value={confirm} onChange={e => setConfirm(e.target.value)} placeholder={t('auth.confirmPlaceholder')}
                className="w-full px-3 py-2 text-sm bg-surface-container-low border border-border-light rounded-lg text-text-main placeholder:text-text-muted focus:outline-none focus:border-primary transition-colors" />
            </div>
            <button type="submit"
              className="w-full bg-primary text-surface text-sm font-bold py-2 rounded-lg hover:opacity-90 transition-opacity">{t('auth.register')}</button>
          </form>
        </div>
