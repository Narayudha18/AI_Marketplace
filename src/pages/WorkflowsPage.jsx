...
export default function WorkflowsPage() {
  ...
  const [selectedCategories, setSelectedCategories] = useState(['All Workflows'])
  ...
  const [appliedCategories, setAppliedCategories] = useState(['All Workflows'])
  ...
  const toggleCategory = (cat) => {
    if (cat === 'All Workflows') {
      setSelectedCategories(['All Workflows'])
      return
    }
    let next = selectedCategories.filter(c => c !== 'All Workflows')
    if (next.includes(cat)) {
      next = next.filter(c => c !== cat)
    } else {
      next.push(cat)
    }
    setSelectedCategories(next.length === 0 ? ['All Workflows'] : next)
  }
  ...
  const resetFilters = () => {
    setSearchQuery(''); setSidebarSearch(''); setSelectedCategories(['All Workflows']); setPriceRange('All Prices'); setSortBy('Newest')
    setAppliedSearch(''); setAppliedSidebar(''); setAppliedCategories(['All Workflows']); setAppliedPrice('All Prices'); setAppliedSort('Newest')
  }

  const filteredWorkflows = workflows.filter(t => {
    ...
    if (!appliedCategories.includes('All Workflows') && !appliedCategories.includes(t.category)) return false
  ...
  return (
    <>
      <div className="bg-gradient-to-r from-primary-container to-blue-600 text-on-primary-container px-6 py-2.5 text-center text-xs font-semibold flex justify-center items-center gap-3">
        ...
        <span>Automation workflows — connect tools, automate processes, ship faster.</span>
      ...
      <div className="bg-surface border-b border-border-light">
        <div className="max-w-[1440px] mx-auto px-6 h-11 flex items-center gap-1 overflow-x-auto">
          {['All Workflows', 'Data Engineering', 'Customer Success', 'DevOps', 'Content Safety', 'Finance', 'Marketing', 'Machine Learning', 'Sales', 'Operations', 'E-commerce', 'HR'].map(item => {
            const slug = item === 'All Workflows' ? '' : toSlug(item)
            const target = slug ? `/ai-workflows/c/${slug}` : '/ai-workflows'
      ...
      <main className="w-full max-w-[1440px] mx-auto pb-16">
        <section className="px-6 py-16 flex flex-col lg:flex-row items-center gap-10">
          <div className="w-full lg:w-1/2 flex flex-col gap-6">
            ...
            <p className="text-[15px] text-text-muted leading-relaxed max-w-xl">
              Pre-built automation sequences that connect your favourite tools. From data pipelines to customer onboarding.
            </p>
            <div className="flex w-full max-w-lg bg-surface rounded-lg shadow-sm border border-border-light p-1">
              <input type="text" placeholder="e.g. Data pipeline" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
          ...
          <div className="w-full lg:w-1/2 relative h-[400px]">
            <img
              src="https://picsum.photos/seed/workflows-hero/600/400"
              alt="Workflows Marketplace"
        ...
        <section className="px-6 py-10 bg-surface-container-low rounded-3xl mx-6 my-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: 'account_tree', title: 'Data Engineering', desc: 'ETL pipelines & data orchestration', tags: ['Airflow', 'dbt', 'Snowflake'] },
              { icon: 'support_agent', title: 'Customer Success', desc: 'Onboarding flows & engagement', tags: ['Zapier', 'SendGrid', 'HubSpot'] },
              { icon: 'cloud_sync', title: 'DevOps', desc: 'CI/CD, incident response & monitoring', tags: ['Docker', 'K8s', 'PagerDuty'] },
              { icon: 'payments', title: 'Finance & Sales', desc: 'Invoicing, lead scoring & approvals', tags: ['QuickBooks', 'Salesforce', 'DocuSign'] },
              { icon: 'campaign', title: 'Marketing', desc: 'Content calendars & social automation', tags: ['Buffer', 'Canva', 'ChatGPT'] },
              { icon: 'inventory_2', title: 'E-commerce', desc: 'Order fulfillment & inventory sync', tags: ['Shopify', 'Stripe', 'ShipStation'] },
              ...
              <div key={cat.title}
                ...
                <img
                  src={`https://picsum.photos/seed/workflow-${cat.icon}/400/120`}
                  alt={cat.title}
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            <aside className="w-full lg:w-72 flex-shrink-0">
              <div className="bg-surface rounded-xl border border-border-light p-5 sticky top-4">
                <div className="flex items-center gap-2 bg-surface-container-low rounded-lg px-3 py-2.5 border border-border-light mb-6">
                  ...
                  <input type="text" placeholder="Search workflows..." value={sidebarSearch} onChange={e => setSidebarSearch(e.target.value)}
                    onKeyDown={e => e.key === 'Enter' && applyFilters()}
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2.5">
                    {['All Workflows', 'Data Engineering', 'Customer Success', 'DevOps', 'Content Safety', 'Finance', 'Marketing', 'Machine Learning', 'Sales', 'Operations', 'E-commerce', 'HR'].map((cat) => (
                ...
                <div className="mb-6">
                  <h4 className="text-xs font-semibold text-text-main mb-3 uppercase tracking-wider">Difficulty</h4>
                  <div className="space-y-2.5">
                    {['All Levels', 'Beginner', 'Intermediate', 'Advanced'].map((level) => (
                      <label key={level} className="flex items-center gap-2.5 cursor-pointer group">
                        <input type="radio" name="difficulty" checked={priceRange === level} onChange={() => setPriceRange(level)}
                          className="w-4 h-4 border-border-light text-primary focus:ring-primary" />
                        <span className="text-xs font-medium text-text-muted group-hover:text-text-main transition-colors">{level}</span>
                      </label>
                    ))}
                  </div>
                </div>
            ...
            <div ref={productRef} className="flex-1">
              <div className="flex items-center justify-between mb-6">
                <span className="text-xs font-medium text-text-muted">{filteredWorkflows.length} workflows found</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                {filteredWorkflows.slice(0, visibleCount).map((t) => (
                  <Link key={t.name} to={`/ai-workflows/${toSlug(t.name)}`}
                    ...
                    <div className="relative h-40 overflow-hidden bg-surface-container-low">
                      ...
                      <button onClick={e => { e.preventDefault(); e.stopPropagation(); toggleFavorite(toSlug(t.name), 'ai-workflows') }}
                        ...
                        <span className={`material-symbols-outlined ${isFavorite(toSlug(t.name), 'ai-workflows') ? 'text-red-500' : 'text-text-muted'}`} style={{ fontSize: 18 }}>{isFavorite(toSlug(t.name), 'ai-workflows') ? 'favorite' : 'favorite_border'}</span>
                      </button>
                    </div>
                    <div className="p-4 flex flex-col flex-1">
                      ...
                      <div className="flex items-center gap-2 text-[11px] text-text-muted mb-3">
                        <span className="bg-surface-container-low px-2 py-0.5 rounded">{t.steps} steps</span>
                        <span className="bg-surface-container-low px-2 py-0.5 rounded">{t.difficulty}</span>
                      </div>
                      <div className="flex flex-wrap gap-1 mb-3">
                        {t.tools.slice(0, 3).map(tool => (
                          <span key={tool} className="text-[10px] bg-primary/10 text-primary px-2 py-0.5 rounded">{tool}</span>
                        ))}
                      </div>
                      <div className="mt-auto flex items-center justify-between border-t border-border-light pt-3">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); navigate(`/ai-workflows/${toSlug(t.name)}/preview`) }} className="px-3 py-1.5 border border-primary text-primary rounded hover:bg-primary hover:text-surface transition-colors text-[11px] font-medium">
                            Preview
                          </button>
                        </div>
                      </div>
        ...
        <section className="px-6 py-16 my-6 bg-surface-container-lowest border-y border-border-light">
          <div className="flex flex-col lg:flex-row gap-8">
            <div className="w-full lg:w-1/3 flex flex-col justify-center border border-border-light border-dashed rounded-xl p-8 bg-surface">
              <h2 className="text-[24px] font-semibold text-text-main mb-4">Featured Workflows</h2>
              <p className="text-[15px] text-text-muted leading-relaxed mb-8">
                Our editors spotlight the most impactful automation workflows every month — from DevOps to marketing.
              </p>
            ...
            <div className="w-full lg:w-2/3 grid grid-cols-1 sm:grid-cols-2 gap-6">
              <img
                src="https://picsum.photos/seed/featured-workflows/800/200"
                alt="Featured Workflows"
      ...
      <footer className="bg-text-main text-surface w-full py-10 px-6 grid grid-cols-2 md:grid-cols-4 gap-6 border-t border-outline">
        <div className="col-span-2 md:col-span-1 flex flex-col gap-4">
          ...
          <p className="text-[15px] text-secondary-fixed-dim mt-4 leading-relaxed">
            &copy; 2026 AI Agents Marketplace. All rights reserved. 12k+ workflows available.
          </p>
        </div>
        ...
        <div className="col-span-2 md:col-span-1 flex flex-col gap-6 justify-end items-start md:items-end mt-8 md:mt-0">
          <div className="text-left md:text-right">
            ...
            <div className="text-[11px] font-medium text-secondary-fixed-dim uppercase tracking-wider">Workflows Available</div>
          </div>
          <div className="text-left md:text-right">
            <div className="text-[24px] font-semibold text-surface mb-1">89,450</div>
            <div className="text-[11px] font-medium text-secondary-fixed-dim uppercase tracking-wider">Workflows Sold</div>
          </div>
        </div>
      </footer>
