...
export default function AgentsPage() {
  ...
  const [selectedCategories, setSelectedCategories] = useState(['All Agents'])
  ...
  const [appliedCategories, setAppliedCategories] = useState(['All Agents'])
  ...
  const toggleCategory = (cat) => {
    if (cat === 'All Agents') {
      setSelectedCategories(['All Agents'])
      return
    }
    let next = selectedCategories.filter(c => c !== 'All Agents')
    if (next.includes(cat)) {
      next = next.filter(c => c !== cat)
    } else {
      next.push(cat)
    }
    setSelectedCategories(next.length === 0 ? ['All Agents'] : next)
  }
  ...
  const resetFilters = () => {
    setSearchQuery(''); setSidebarSearch(''); setSelectedCategories(['All Agents']); setPriceRange('All Prices'); setSortBy('Newest')
    setAppliedSearch(''); setAppliedSidebar(''); setAppliedCategories(['All Agents']); setAppliedPrice('All Prices'); setAppliedSort('Newest')
  }

  const filteredAgents = agents.filter(t => {
    ...
    if (!appliedCategories.includes('All Agents') && !appliedCategories.includes(t.category)) return false
  ...
  return (
    <>
      <div className="bg-gradient-to-r from-primary-container to-blue-600 text-on-primary-container px-6 py-2.5 text-center text-xs font-semibold flex justify-center items-center gap-3">
        ...
        <span>Autonomous AI agents ready to work — deploy, connect, and automate.</span>
      ...
      <div className="bg-surface border-b border-border-light">
        <div className="max-w-[1440px] mx-auto px-6 h-11 flex items-center gap-1 overflow-x-auto">
          {['All Agents', 'Developer Tools', 'Customer Service', 'Research', 'Data Science', 'Marketing', 'DevOps', 'Legal', 'HR', 'Finance', 'Content Creation'].map(item => {
            const slug = item === 'All Agents' ? '' : toSlug(item)
            const target = slug ? `/ai-agents/c/${slug}` : '/ai-agents'
      ...
      <main className="w-full max-w-[1440px] mx-auto pb-16">
        <section className="px-6 py-16 flex flex-col lg:flex-row items-center gap-10">
          <div className="w-full lg:w-1/2 flex flex-col gap-6">
            ...
            <p className="text-[15px] text-text-muted leading-relaxed max-w-xl">
              Deploy AI agents that work alongside your team — code review, support, research, and more.
            </p>
            <div className="flex w-full max-w-lg bg-surface rounded-lg shadow-sm border border-border-light p-1">
              <input type="text" placeholder="e.g. Code review assistant" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
          ...
          <div className="w-full lg:w-1/2 relative h-[400px]">
            <img
              src="https://picsum.photos/seed/agents-hero/600/400"
              alt="Agents Marketplace"
        ...
        <section className="px-6 py-10 bg-surface-container-low rounded-3xl mx-6 my-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: 'code', title: 'Developer Tools', desc: 'Code review, QA testing, docs', tags: ['VS Code', 'GitHub', 'API'] },
              { icon: 'headset_mic', title: 'Customer Service', desc: '24/7 support, ticket triage', tags: ['Zendesk', 'Slack', 'Web'] },
              { icon: 'science', title: 'Research', desc: 'Paper synthesis, citation mgmt', tags: ['Search', 'Summary', 'API'] },
              { icon: 'analytics', title: 'Data Science', desc: 'SQL queries, charts, trends', tags: ['SQL', 'Slack', 'Dashboards'] },
              { icon: 'campaign', title: 'Marketing', desc: 'Email campaigns, social mgmt', tags: ['Email', 'Social', 'SEO'] },
              { icon: 'monitoring', title: 'DevOps & Ops', desc: 'Monitoring, remediation, runbooks', tags: ['PagerDuty', 'K8s', 'API'] },
              ...
              <div key={cat.title}
                ...
                <img
                  src={`https://picsum.photos/seed/agent-${cat.icon}/400/120`}
                  alt={cat.title}
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            <aside className="w-full lg:w-72 flex-shrink-0">
              <div className="bg-surface rounded-xl border border-border-light p-5 sticky top-4">
                <div className="flex items-center gap-2 bg-surface-container-low rounded-lg px-3 py-2.5 border border-border-light mb-6">
                  ...
                  <input type="text" placeholder="Search agents..." value={sidebarSearch} onChange={e => setSidebarSearch(e.target.value)}
                    onKeyDown={e => e.key === 'Enter' && applyFilters()}
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2.5">
                    {['All Agents', 'Developer Tools', 'Customer Service', 'Research', 'Data Science', 'Marketing', 'DevOps', 'Legal', 'HR', 'Finance', 'Content Creation'].map((cat) => (
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2.5">
                    {['All Platforms', 'VS Code', 'Web', 'API', 'Slack', 'Mobile'].map((plat) => (
                      <label key={plat} className="flex items-center gap-2.5 cursor-pointer group">
                        <input type="radio" name="platform" checked={priceRange === plat} onChange={() => setPriceRange(plat)}
                          className="w-4 h-4 border-border-light text-primary focus:ring-primary" />
                        <span className="text-xs font-medium text-text-muted group-hover:text-text-main transition-colors">{plat}</span>
                      </label>
                    ))}
                  </div>
                </div>
            ...
            <div ref={productRef} className="flex-1">
              <div className="flex items-center justify-between mb-6">
                <span className="text-xs font-medium text-text-muted">{filteredAgents.length} agents found</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                {filteredAgents.slice(0, visibleCount).map((t) => (
                  <Link key={t.name} to={`/ai-agents/${toSlug(t.name)}`}
                    ...
                    <div className="relative h-40 overflow-hidden bg-surface-container-low">
                      ...
                      <button onClick={e => { e.preventDefault(); e.stopPropagation(); toggleFavorite(toSlug(t.name), 'ai-agents') }}
                        ...
                        <span className={`material-symbols-outlined ${isFavorite(toSlug(t.name), 'ai-agents') ? 'text-red-500' : 'text-text-muted'}`} style={{ fontSize: 18 }}>{isFavorite(toSlug(t.name), 'ai-agents') ? 'favorite' : 'favorite_border'}</span>
                      </button>
                    </div>
                    <div className="p-4 flex flex-col flex-1">
                      ...
                      <div className="mt-auto flex items-center justify-between border-t border-border-light pt-3">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); navigate(`/ai-agents/${toSlug(t.name)}/preview`) }} className="px-3 py-1.5 border border-primary text-primary rounded hover:bg-primary hover:text-surface transition-colors text-[11px] font-medium">
                            Preview
                          </button>
                        </div>
                      </div>
        ...
        <section className="px-6 py-16 my-6 bg-surface-container-lowest border-y border-border-light">
          <div className="flex flex-col lg:flex-row gap-8">
            <div className="w-full lg:w-1/3 flex flex-col justify-center border border-border-light border-dashed rounded-xl p-8 bg-surface">
              <h2 className="text-[24px] font-semibold text-text-main mb-4">Featured Agents</h2>
              <p className="text-[15px] text-text-muted leading-relaxed mb-8">
                Monthly curated selection of the best AI agents — from developer tools to customer service bots.
              </p>
            ...
            <div className="w-full lg:w-2/3 grid grid-cols-1 sm:grid-cols-2 gap-6">
              <img
                src="https://picsum.photos/seed/featured-agents/800/200"
                alt="Featured Agents"
      ...
      <footer className="bg-text-main text-surface w-full py-10 px-6 grid grid-cols-2 md:grid-cols-4 gap-6 border-t border-outline">
        <div className="col-span-2 md:col-span-1 flex flex-col gap-4">
          ...
          <p className="text-[15px] text-secondary-fixed-dim mt-4 leading-relaxed">
            &copy; 2026 AI Agents Marketplace. All rights reserved. 12k+ agents available.
          </p>
        </div>
        ...
        <div className="col-span-2 md:col-span-1 flex flex-col gap-6 justify-end items-start md:items-end mt-8 md:mt-0">
          <div className="text-left md:text-right">
            ...
            <div className="text-[11px] font-medium text-secondary-fixed-dim uppercase tracking-wider">Agents Available</div>
          </div>
          <div className="text-left md:text-right">
            <div className="text-[24px] font-semibold text-surface mb-1">89,450</div>
            <div className="text-[11px] font-medium text-secondary-fixed-dim uppercase tracking-wider">Agents Sold</div>
          </div>
        </div>
      </footer>
