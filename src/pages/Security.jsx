...
export default function Security() {
  ...
  const [selectedCategories, setSelectedCategories] = useState(['All Security'])
  ...
  const [appliedCategories, setAppliedCategories] = useState(['All Security'])
  ...
  const toggleCategory = (cat) => {
    if (cat === 'All Security') {
      setSelectedCategories(['All Security'])
      return
    }
    let next = selectedCategories.filter(c => c !== 'All Security')
    if (next.includes(cat)) {
      next = next.filter(c => c !== cat)
    } else {
      next.push(cat)
    }
    setSelectedCategories(next.length === 0 ? ['All Security'] : next)
  }
  ...
  const resetFilters = () => {
    setSearchQuery(''); setSidebarSearch(''); setSelectedCategories(['All Security']); setPriceRange('All Prices'); setSortBy('Newest')
    setAppliedSearch(''); setAppliedSidebar(''); setAppliedCategories(['All Security']); setAppliedPrice('All Prices'); setAppliedSort('Newest')
  }

  const filteredTemplates = security.filter(t => {
    ...
    if (!appliedCategories.includes('All Security') && !appliedCategories.includes(t.category)) return false
  ...
  return (
    <>
      <div className="bg-gradient-to-r from-primary-container to-blue-600 text-on-primary-container px-6 py-2.5 text-center text-xs font-semibold flex justify-center items-center gap-3">
        ...
        <span>Guardrails, red teaming, and compliance for your AI stack.</span>
      ...
      <div className="bg-surface border-b border-border-light">
        <div className="max-w-[1440px] mx-auto px-6 h-11 flex items-center gap-1 overflow-x-auto">
          {['All Security', 'Guardrails', 'Red Teaming', 'Compliance', 'Data Privacy'].map(item => {
            const slug = item === 'All Security' ? '' : toSlug(item)
            const target = slug ? `/security/c/${slug}` : '/security'
      ...
      <main className="w-full max-w-[1440px] mx-auto pb-16">
        <section className="px-6 py-16 flex flex-col lg:flex-row items-center gap-10">
          <div className="w-full lg:w-1/2 flex flex-col gap-6">
            ...
            <p className="text-[15px] text-text-muted leading-relaxed max-w-xl">
              Protect your AI applications with guardrails, red teaming, compliance frameworks, and data privacy tools.
            </p>
            <div className="flex w-full max-w-lg bg-surface rounded-lg shadow-sm border border-border-light p-1">
              <input type="text" placeholder="e.g. Guardrails suite" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
          ...
          <div className="w-full lg:w-1/2 relative h-[400px]">
            <img
              src="https://picsum.photos/seed/security-hero/600/400"
              alt="Security Marketplace"
        ...
        <section className="px-6 py-10 bg-surface-container-low rounded-3xl mx-6 my-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: 'shield', title: 'Guardrails', desc: 'Content filters & safety boundaries', tags: ['Newest', 'Bestsellers', 'Safety', 'Policy', 'Moderation'] },
              { icon: 'bug_report', title: 'Red Teaming', desc: 'Adversarial testing & attack sims', tags: ['Newest', 'Bestsellers', 'Pen-test', 'Simulation', 'Adversarial'] },
              { icon: 'verified', title: 'Compliance', desc: 'Audit frameworks & regulation tools', tags: ['Newest', 'Bestsellers', 'GDPR', 'SOC2', 'HIPAA'] },
              { icon: 'lock', title: 'Data Privacy', desc: 'Anonymization & privacy management', tags: ['Newest', 'Bestsellers', 'PII', 'Anonymize', 'GDPR'] },
              { icon: 'gavel', title: 'Governance', desc: 'Policy management & access control', tags: ['Newest', 'Bestsellers', 'RBAC', 'Policy', 'Audit'] },
              { icon: 'verified_user', title: 'Certification', desc: 'Compliance certifications & reports', tags: ['Newest', 'Bestsellers', 'SOC2', 'ISO', 'HIPAA'] },
              ...
              <div key={cat.title}
                ...
                <img
                  src={`https://picsum.photos/seed/security-${cat.icon}/400/120`}
                  alt={cat.title}
          ...
          <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="bg-surface rounded-xl border border-border-light p-5 sticky top-4">
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2.5">
                    {['All Security', 'Guardrails', 'Red Teaming', 'Compliance', 'Data Privacy'].map((cat) => (
            ...
            <div ref={productRef} className="flex-1">
              <div className="flex items-center justify-between mb-6">
                <span className="text-xs font-medium text-text-muted">{filteredTemplates.length} security tools found</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                {filteredTemplates.slice(0, visibleCount).map((t) => (
                  <Link key={t.title} to={`/security/${toSlug(t.title)}`}
                    ...
                    <div className="relative h-40 overflow-hidden bg-surface-container-low">
                      ...
                      <button onClick={e => { e.preventDefault(); e.stopPropagation(); toggleFavorite(toSlug(t.title), 'security') }}
                        ...
                        <span className={`material-symbols-outlined ${isFavorite(toSlug(t.title), 'security') ? 'text-red-500' : 'text-text-muted'}`} style={{ fontSize: 18 }}>{isFavorite(toSlug(t.title), 'security') ? 'favorite' : 'favorite_border'}</span>
                      </button>
                    </div>
                    <div className="p-4 flex flex-col flex-1">
                      ...
                      <div className="mt-auto flex items-center justify-between border-t border-border-light pt-3">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); navigate(`/security/${toSlug(t.title)}/preview`) }} className="px-3 py-1.5 border border-primary text-primary rounded hover:bg-primary hover:text-surface transition-colors text-[11px] font-medium">
                            Preview
                          </button>
                        </div>
                      </div>
        ...
        <section className="px-6 py-16 my-6 bg-surface-container-lowest border-y border-border-light">
          <div className="flex flex-col lg:flex-row gap-8">
            <div className="w-full lg:w-1/3 flex flex-col justify-center border border-border-light border-dashed rounded-xl p-8 bg-surface">
              <h2 className="text-[24px] font-semibold text-text-main mb-4">Featured Security</h2>
              <p className="text-[15px] text-text-muted leading-relaxed mb-8">
                Every month, our curators pick the best new security tools — from guardrails to compliance frameworks.
              </p>
            ...
            <div className="w-full lg:w-2/3 grid grid-cols-1 sm:grid-cols-2 gap-6">
              <img
                src="https://picsum.photos/seed/featured-security/800/200"
                alt="Featured Security"
      ...
      <footer className="bg-text-main text-surface w-full py-10 px-6 grid grid-cols-2 md:grid-cols-4 gap-6 border-t border-outline">
        <div className="col-span-2 md:col-span-1 flex flex-col gap-4">
          ...
          <p className="text-[15px] text-secondary-fixed-dim mt-4 leading-relaxed">
            &copy; 2026 AI Agents Marketplace. All rights reserved. 8 security tools available.
          </p>
        </div>
        ...
        <div className="col-span-2 md:col-span-1 flex flex-col gap-6 justify-end items-start md:items-end mt-8 md:mt-0">
          <div className="text-left md:text-right">
            <div className="text-[24px] font-semibold text-surface mb-1">2,668</div>
            <div className="text-[11px] font-medium text-secondary-fixed-dim uppercase tracking-wider">Threats Blocked</div>
          </div>
          <div className="text-left md:text-right">
            <div className="text-[24px] font-semibold text-surface mb-1">1,890</div>
            <div className="text-[11px] font-medium text-secondary-fixed-dim uppercase tracking-wider">Compliance Checks</div>
          </div>
        </div>
      </footer>
