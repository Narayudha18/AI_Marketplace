...
export default function TokensPage() {
  ...
  const [selectedCategories, setSelectedCategories] = useState(['All Tokens'])
  ...
  const [appliedCategories, setAppliedCategories] = useState(['All Tokens'])
  ...
  const toggleCategory = (cat) => {
    if (cat === 'All Tokens') {
      setSelectedCategories(['All Tokens'])
      return
    }
    let next = selectedCategories.filter(c => c !== 'All Tokens')
    if (next.includes(cat)) {
      next = next.filter(c => c !== cat)
    } else {
      next.push(cat)
    }
    setSelectedCategories(next.length === 0 ? ['All Tokens'] : next)
  }
  ...
  const resetFilters = () => {
    setSearchQuery(''); setSidebarSearch(''); setSelectedCategories(['All Tokens']); setPriceRange('All Prices'); setSortBy('Newest')
    setAppliedSearch(''); setAppliedSidebar(''); setAppliedCategories(['All Tokens']); setAppliedPrice('All Prices'); setAppliedSort('Newest')
  }
  ...
  const filteredTokens = tokens.filter(t => {
    ...
    if (!appliedCategories.includes('All Tokens') && !appliedCategories.includes(t.category)) return false
  ...
  return (
    <>
      <div className="bg-gradient-to-r from-primary-container to-blue-600 text-on-primary-container px-6 py-2.5 text-center text-xs font-semibold flex justify-center items-center gap-3">
        ...
        <span>AI tokens & API credits — pay-as-you-go access to top models.</span>
      ...
      <div className="bg-surface border-b border-border-light">
        <div className="max-w-[1440px] mx-auto px-6 h-11 flex items-center gap-1 overflow-x-auto">
          {['All Tokens', 'LLM Tokens', 'Image Generation', 'Audio', 'Search'].map(item => {
            const slug = item === 'All Tokens' ? '' : toSlug(item)
            const target = slug ? `/ai-tokens/c/${slug}` : '/ai-tokens'
      ...
      <main className="w-full max-w-[1440px] mx-auto pb-16">
        <section className="px-6 py-16 flex flex-col lg:flex-row items-center gap-10">
          <div className="w-full lg:w-1/2 flex flex-col gap-6">
            <h1 className="text-[30px] md:text-[38px] font-bold leading-[1.2] tracking-tight text-text-main">
              AI tokens & API credits marketplace
            </h1>
            <p className="text-[15px] text-text-muted leading-relaxed max-w-xl">
              Buy tokens from OpenAI, Anthropic, Google, and more. Pay-as-you-go access without the commitment.
            </p>
            <div className="flex w-full max-w-lg bg-surface rounded-lg shadow-sm border border-border-light p-1">
              <input type="text" placeholder="e.g. GPT-4 tokens" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
          ...
          <div className="w-full lg:w-1/2 relative h-[400px]">
            <img
              src="https://picsum.photos/seed/tokens-hero/600/400"
              alt="Tokens Marketplace"
        ...
        <section className="px-6 py-10 bg-surface-container-low rounded-3xl mx-6 my-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: 'token', title: 'LLM Tokens', desc: 'GPT-4, Claude, Gemini & more', tags: ['OpenAI', 'Anthropic', 'Google'] },
              { icon: 'image', title: 'Image Generation', desc: 'DALL-E, SDXL, Midjourney', tags: ['DALL-E', 'SDXL', 'MJ'] },
              { icon: 'mic', title: 'Audio & Speech', desc: 'Whisper, TTS, voice synthesis', tags: ['Whisper', 'Eleven', 'TTS'] },
              { icon: 'search', title: 'Search & Embeddings', desc: 'Perplexity, vector embeddings', tags: ['Search', 'Embed', 'RAG'] },
              ...
              <div key={cat.title}
                ...
                <img
                  src={`https://picsum.photos/seed/token-${cat.icon}/400/120`}
                  alt={cat.title}
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            <aside className="w-full lg:w-72 flex-shrink-0">
              <div className="bg-surface rounded-xl border border-border-light p-5 sticky top-4">
                <div className="flex items-center gap-2 bg-surface-container-low rounded-lg px-3 py-2.5 border border-border-light mb-6">
                  ...
                  <input type="text" placeholder="Search tokens..." value={sidebarSearch} onChange={e => setSidebarSearch(e.target.value)}
                    onKeyDown={e => e.key === 'Enter' && applyFilters()}
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2.5">
                    {['All Tokens', 'LLM Tokens', 'Image Generation', 'Audio', 'Search'].map((cat) => (
            ...
            <div ref={productRef} className="flex-1">
              <div className="flex items-center justify-between mb-6">
                <span className="text-xs font-medium text-text-muted">{filteredTokens.length} token packs found</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                {filteredTokens.slice(0, visibleCount).map((t) => (
                  <Link key={t.name} to={`/ai-tokens/${toSlug(t.name)}`}
                    ...
                    <div className="relative h-40 overflow-hidden bg-surface-container-low">
                      ...
                      <button onClick={e => { e.preventDefault(); e.stopPropagation(); toggleFavorite(toSlug(t.name), 'ai-tokens') }}
                        ...
                        <span className={`material-symbols-outlined ${isFavorite(toSlug(t.name), 'ai-tokens') ? 'text-red-500' : 'text-text-muted'}`} style={{ fontSize: 18 }}>{isFavorite(toSlug(t.name), 'ai-tokens') ? 'favorite' : 'favorite_border'}</span>
                      </button>
                    </div>
                    <div className="p-4 flex flex-col flex-1">
                      ...
                      <div className="flex items-center gap-2 text-[11px] text-text-muted mb-3">
                        <span className="bg-surface-container-low px-2 py-0.5 rounded">{t.tokenCount}</span>
                      </div>
                      <div className="flex items-center gap-1 mb-3">
                        <span className="text-[10px] bg-primary/10 text-primary px-2 py-0.5 rounded">{t.pricePerToken}</span>
                      </div>
                      <div className="mt-auto flex items-center justify-between border-t border-border-light pt-3">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); navigate(`/ai-tokens/${toSlug(t.name)}/preview`) }} className="px-3 py-1.5 border border-primary text-primary rounded hover:bg-primary hover:text-surface transition-colors text-[11px] font-medium">
                            Preview
                          </button>
                        </div>
                      </div>
        ...
        <section className="px-6 py-16 my-6 bg-surface-container-lowest border-y border-border-light">
          <div className="flex flex-col lg:flex-row gap-8">
            <div className="w-full lg:w-1/3 flex flex-col justify-center border border-border-light border-dashed rounded-xl p-8 bg-surface">
              <h2 className="text-[24px] font-semibold text-text-main mb-4">Featured Tokens</h2>
              <p className="text-[15px] text-text-muted leading-relaxed mb-8">
                Best-value token packs hand-picked by our team — from LLM inference to image generation credits.
              </p>
            ...
            <div className="w-full lg:w-2/3 grid grid-cols-1 sm:grid-cols-2 gap-6">
              <img
                src="https://picsum.photos/seed/featured-tokens/800/200"
                alt="Featured Tokens"
      ...
      <footer className="bg-text-main text-surface w-full py-10 px-6 grid grid-cols-2 md:grid-cols-4 gap-6 border-t border-outline">
        <div className="col-span-2 md:col-span-1 flex flex-col gap-4">
          ...
          <p className="text-[15px] text-secondary-fixed-dim mt-4 leading-relaxed">
            &copy; 2026 AI Agents Marketplace. All rights reserved. 12k+ tokens available.
          </p>
        </div>
        ...
        <div className="col-span-2 md:col-span-1 flex flex-col gap-6 justify-end items-start md:items-end mt-8 md:mt-0">
          <div className="text-left md:text-right">
            ...
            <div className="text-[11px] font-medium text-secondary-fixed-dim uppercase tracking-wider">Tokens Available</div>
          </div>
          <div className="text-left md:text-right">
            <div className="text-[24px] font-semibold text-surface mb-1">89,450</div>
            <div className="text-[11px] font-medium text-secondary-fixed-dim uppercase tracking-wider">Tokens Sold</div>
          </div>
        </div>
      </footer>
