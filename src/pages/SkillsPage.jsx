...
export default function SkillsPage() {
  ...
  const [appliedCategories, setAppliedCategories] = useState(['All Skills'])
  ...
  const toggleCategory = (cat) => {
    if (cat === 'All Skills') {
      setSelectedCategories(['All Skills'])
      return
    }
    let next = selectedCategories.filter(c => c !== 'All Skills')
    if (next.includes(cat)) {
      next = next.filter(c => c !== cat)
    } else {
      next.push(cat)
    }
    setSelectedCategories(next.length === 0 ? ['All Skills'] : next)
  }
  ...
  const resetFilters = () => {
    setSearchQuery(''); setSidebarSearch(''); setSelectedCategories(['All Skills']); setPriceRange('All Prices'); setSortBy('Newest')
    setAppliedSearch(''); setAppliedSidebar(''); setAppliedCategories(['All Skills']); setAppliedPrice('All Prices'); setAppliedSort('Newest')
  }

  const filteredSkills = skills.filter(t => {
    ...
    if (!appliedCategories.includes('All Skills') && !appliedCategories.includes(t.category)) return false
      ...
      <main className="w-full max-w-[1440px] mx-auto pb-16">
        <section className="px-6 py-16 flex flex-col lg:flex-row items-center gap-10">
          <div className="w-full lg:w-1/2 flex flex-col gap-6">
            <h1 className="text-[30px] md:text-[38px] font-bold leading-[1.2] tracking-tight text-text-main">
              AI skill packs for every use case
            </h1>
          ...
          <div className="w-full lg:w-1/2 relative h-[400px]">
            <img
              src="https://picsum.photos/seed/skills-hero/600/400"
        ...
        <section className="px-6 py-10 bg-surface-container-low rounded-3xl mx-6 my-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: 'analytics', title: 'Data Science', desc: 'Pandas, numpy, forecasting skills', tags: ['Python', 'Statistics', 'ML'] },
              { icon: 'terminal', title: 'Developer Tools', desc: 'Regex, Markdown, API tooling', tags: ['Regex', 'Docs', 'GraphQL'] },
              { icon: 'cloud', title: 'DevOps & Automation', desc: 'Shell scripts, CI/CD, scraping', tags: ['Bash', 'Scraping', 'Docker'] },
              { icon: 'visibility', title: 'Computer Vision', desc: 'Image processing & analysis', tags: ['OpenCV', 'PIL', 'Vision'] },
              { icon: 'link', title: 'Integrations', desc: 'API connectors & middleware', tags: ['REST', 'GraphQL', 'Webhook'] },
              { icon: 'database', title: 'Databases & SQL', desc: 'Query gen, schema design', tags: ['SQL', 'NoSQL', 'Optimisation'] },
              ...
              <div key={cat.title}
                ...
                <img
                  src={`https://picsum.photos/seed/skill-${cat.icon}/400/120`}
                  alt={cat.title}
            ...
            <aside className="w-full lg:w-72 flex-shrink-0">
              <div className="bg-surface rounded-xl border border-border-light p-5 sticky top-4">
                <div className="flex items-center gap-2 bg-surface-container-low rounded-lg px-3 py-2.5 border border-border-light mb-6">
                  ...
                  <input type="text" placeholder="Search skills..." value={sidebarSearch} onChange={e => setSidebarSearch(e.target.value)}
                    onKeyDown={e => e.key === 'Enter' && applyFilters()}
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2.5">
                    {['All Skills', 'Data Science', 'Automation', 'Machine Learning', 'Databases', 'Integration', 'Developer Tools', 'DevOps', 'Computer Vision', 'Blockchain'].map((cat) => (
                  ...
                  <Link key={t.name} to={`/ai-skills/${toSlug(t.name)}`}
                    ...
                    <div className="relative h-40 overflow-hidden bg-surface-container-low">
                      ...
                      <button onClick={e => { e.preventDefault(); e.stopPropagation(); toggleFavorite(toSlug(t.name), 'ai-skills') }}
                        ...
                        <span className={`material-symbols-outlined ${isFavorite(toSlug(t.name), 'ai-skills') ? 'text-red-500' : 'text-text-muted'}`} style={{ fontSize: 18 }}>{isFavorite(toSlug(t.name), 'ai-skills') ? 'favorite' : 'favorite_border'}</span>
                      </button>
                    </div>
                    <div className="p-4 flex flex-col flex-1">
                      ...
                      <div className="flex items-center gap-2 text-[11px] text-text-muted mb-3">
                        <span className="bg-surface-container-low px-2 py-0.5 rounded">{t.fileFormat}</span>
                      ...
                      <div className="mt-auto flex items-center justify-between border-t border-border-light pt-3">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); navigate(`/ai-skills/${toSlug(t.name)}/preview`) }} className="px-3 py-1.5 border border-primary text-primary rounded hover:bg-primary hover:text-surface transition-colors text-[11px] font-medium">
                            Preview
                          </button>
                        </div>
                      </div>
        ...
        <section className="px-6 py-16 my-6 bg-surface-container-lowest border-y border-border-light">
          <div className="flex flex-col lg:flex-row gap-8">
            <div className="w-full lg:w-1/3 flex flex-col justify-center border border-border-light border-dashed rounded-xl p-8 bg-surface">
              <h2 className="text-[24px] font-semibold text-text-main mb-4">Featured Skills</h2>
              <p className="text-[15px] text-text-muted leading-relaxed mb-8">
                Each month our team picks the most effective AI skill packs — from data science to DevOps automation.
              </p>
            ...
            <div className="w-full lg:w-2/3 grid grid-cols-1 sm:grid-cols-2 gap-6">
              <img
                src="https://picsum.photos/seed/featured-skills/800/200"
                alt="Featured Skills"
      ...
      <footer className="bg-text-main text-surface w-full py-10 px-6 grid grid-cols-2 md:grid-cols-4 gap-6 border-t border-outline">
        <div className="col-span-2 md:col-span-1 flex flex-col gap-4">
          ...
          <p className="text-[15px] text-secondary-fixed-dim mt-4 leading-relaxed">
            &copy; 2026 AI Agents Marketplace. All rights reserved. 12k+ skills available.
          </p>
        </div>
        ...
        <div className="col-span-2 md:col-span-1 flex flex-col gap-6 justify-end items-start md:items-end mt-8 md:mt-0">
          <div className="text-left md:text-right">
            ...
            <div className="text-[11px] font-medium text-secondary-fixed-dim uppercase tracking-wider">Skills Available</div>
          </div>
          <div className="text-left md:text-right">
            <div className="text-[24px] font-semibold text-surface mb-1">89,450</div>
            <div className="text-[11px] font-medium text-secondary-fixed-dim uppercase tracking-wider">Skills Sold</div>
          </div>
        </div>
      </footer>
