...
<main className="w-full max-w-[1440px] mx-auto pb-16">
        <section className="px-6 py-16 flex flex-col lg:flex-row items-center gap-10">
          <div className="w-full lg:w-1/2 flex flex-col gap-6">
            <h1 className="text-[30px] md:text-[38px] font-bold leading-[1.2] tracking-tight text-text-main">
              {t('listingPages.monitoring.heroTitle')}
            </h1>
            <p className="text-[15px] text-text-muted leading-relaxed max-w-xl">
              {t('listingPages.monitoring.heroDesc')}
            </p>
        ...
        <section ref={gridRef} className="px-6 py-16">
          <h2 className="text-[24px] font-semibold text-text-main mb-8">
            {t('listingPages.monitoring.bannerSection')}
          </h2>
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            <aside className="w-full lg:w-72 flex-shrink-0">
              <div className="bg-surface rounded-xl border border-border-light p-5 sticky top-4">
                ...
                <div className="mb-6">
                  <h4 className="text-xs font-semibold text-text-main mb-3 uppercase tracking-wider">{t('listingPages.monitoring.sidebarCategory')}</h4>
                ...
                <div className="mb-6">
                  <h4 className="text-xs font-semibold text-text-main mb-3 uppercase tracking-wider">{t('listingPages.monitoring.sidebarPrice')}</h4>
                  <div className="space-y-2.5">
                ...
                <div>
                  <h4 className="text-xs font-semibold text-text-main mb-3 uppercase tracking-wider">{t('listingPages.monitoring.sidebarSort')}</h4>
            ...
            <div ref={productRef} className="flex-1">
              <div className="flex items-center justify-between mb-6">
                <span className="text-xs font-medium text-text-muted">{filteredTemplates.length} {t('listingPages.monitoring.results')}</span>
                    ...
                    <div className="p-4 flex flex-col flex-1">
                      ...
                      <div className="mt-auto flex items-center justify-between border-t border-border-light pt-3">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); navigate(`/monitoring/${toSlug(t.title)}/preview`) }} className="px-3 py-1.5 border border-primary text-primary rounded hover:bg-primary hover:text-surface transition-colors text-[11px] font-medium">
                            {t('listingPages.monitoring.button')}
                          </button>
                        </div>
                      </div>
                    </div>
        ...
        <section className="px-6 py-16 my-6 bg-surface-container-lowest border-y border-border-light">
          <div className="flex flex-col lg:flex-row gap-8">
            <div className="w-full lg:w-1/3 flex flex-col justify-center border border-border-light border-dashed rounded-xl p-8 bg-surface">
              <h2 className="text-[24px] font-semibold text-text-main mb-4">{t('listingPages.monitoring.featuredTitle')}</h2>
              <p className="text-[15px] text-text-muted leading-relaxed mb-8">
                {t('listingPages.monitoring.featuredDesc')}
              </p>
            ...
            <div className="w-full lg:w-2/3 grid grid-cols-1 sm:grid-cols-2 gap-6">
              <img
                ...
                alt={t('listingPages.monitoring.featuredTitle')}
      ...
      <footer className="bg-text-main text-surface w-full py-10 px-6 grid grid-cols-2 md:grid-cols-4 gap-6 border-t border-outline">
        <div className="col-span-2 md:col-span-1 flex flex-col gap-4">
          ...
          <p className="text-[15px] text-secondary-fixed-dim mt-4 leading-relaxed">
            {t('listingPages.monitoring.copyright')}
          </p>
        </div>
        ...
        <div className="col-span-2 md:col-span-1 flex flex-col gap-6 justify-end items-start md:items-end mt-8 md:mt-0">
          <div className="text-left md:text-right">
            ...
            <div className="text-[11px] font-medium text-secondary-fixed-dim uppercase tracking-wider">{t('listingPages.monitoring.stats')[0]}</div>
          </div>
          <div className="text-left md:text-right">
            <div className="text-[24px] font-semibold text-surface mb-1">5,267</div>
            <div className="text-[11px] font-medium text-secondary-fixed-dim uppercase tracking-wider">{t('listingPages.monitoring.stats')[1]}</div>
          </div>
        </div>
      </footer>
    </>
