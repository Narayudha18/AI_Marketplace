...
export default function ImageGen() {
  ...
  const [selectedCategories, setSelectedCategories] = useState(['All Image Gen'])
  ...
  const [appliedCategories, setAppliedCategories] = useState(['All Image Gen'])
  ...
  const toggleCategory = (cat) => {
    if (cat === 'All Image Gen') {
      setSelectedCategories(['All Image Gen'])
      return
    }
    let next = selectedCategories.filter(c => c !== 'All Image Gen')
    if (next.includes(cat)) {
      next = next.filter(c => c !== cat)
    } else {
      next.push(cat)
    }
    setSelectedCategories(next.length === 0 ? ['All Image Gen'] : next)
  }
  ...
  const resetFilters = () => {
    setSearchQuery(''); setSidebarSearch(''); setSelectedCategories(['All Image Gen']); setPriceRange('All Prices'); setSortBy('Newest')
    setAppliedSearch(''); setAppliedSidebar(''); setAppliedCategories(['All Image Gen']); setAppliedPrice('All Prices'); setAppliedSort('Newest')
  }

  const filteredTemplates = imageGen.filter(t => {
    ...
    if (!appliedCategories.includes('All Image Gen') && !appliedCategories.includes(t.category)) return false
  ...
  return (
    <>
      <div className="bg-gradient-to-r from-primary-container to-blue-600 text-on-primary-container px-6 py-2.5 text-center text-xs font-semibold flex justify-center items-center gap-3">
        ...
        <span>Premium AI image generation tools & APIs for stunning visuals.</span>
      ...
      <div className="bg-surface border-b border-border-light">
        <div className="max-w-[1440px] mx-auto px-6 h-11 flex items-center gap-1 overflow-x-auto">
          {['All Image Gen', 'Text-to-Image', 'Image Editing', 'Video Gen', '3D Models'].map(item => {
            const slug = item === 'All Image Gen' ? '' : toSlug(item)
            const target = slug ? `/image-gen/c/${slug}` : '/image-gen'
      ...
      <main className="w-full max-w-[1440px] mx-auto pb-16">
        <section className="px-6 py-16 flex flex-col lg:flex-row items-center gap-10">
          <div className="w-full lg:w-1/2 flex flex-col gap-6">
            ...
            <p className="text-[15px] text-text-muted leading-relaxed max-w-xl">
              Generate stunning visuals with text-to-image, image editing, video generation, and 3D model APIs.
            </p>
            <div className="flex w-full max-w-lg bg-surface rounded-lg shadow-sm border border-border-light p-1">
              <input type="text" placeholder="e.g. Text-to-image" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
          ...
          <div className="w-full lg:w-1/2 relative h-[400px]">
            <img
              src="https://picsum.photos/seed/image-gen-hero/600/400"
              alt="Image Gen Marketplace"
        ...
        <section className="px-6 py-10 bg-surface-container-low rounded-3xl mx-6 my-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: 'image', title: 'Text-to-Image', desc: 'Generate images from text prompts', tags: ['Newest', 'Bestsellers', 'SDXL', 'DALL-E', 'Midjourney'] },
              { icon: 'edit', title: 'Image Editing', desc: 'AI-powered photo & image editing', tags: ['Newest', 'Bestsellers', 'Retouch', 'Style', 'Transform'] },
              { icon: 'videocam', title: 'Video Gen', desc: 'Create videos with AI generation', tags: ['Newest', 'Bestsellers', 'Short', 'Motion', 'Animation'] },
              { icon: 'view_in_ar', title: '3D Models', desc: 'Generate 3D assets from text', tags: ['Newest', 'Bestsellers', 'Procedural', 'Voxel', 'Mesh'] },
              ...
              <div key={cat.title}
                ...
                <img
                  src={`https://picsum.photos/seed/image-${cat.icon}/400/120`}
                  alt={cat.title}
            ...
            <aside className="w-full lg:w-72 flex-shrink-0">
              <div className="bg-surface rounded-xl border border-border-light p-5 sticky top-4">
                <div className="flex items-center gap-2 bg-surface-container-low rounded-lg px-3 py-2.5 border border-border-light mb-6">
                  ...
                  <input type="text" placeholder="Search image gen..." value={sidebarSearch} onChange={e => setSidebarSearch(e.target.value)}
                    onKeyDown={e => e.key === 'Enter' && applyFilters()}
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2.5">
                    {['All Image Gen', 'Text-to-Image', 'Image Editing', 'Video Gen', '3D Models'].map((cat) => (
                  ...
                  <Link key={t.title} to={`/image-gen/${toSlug(t.title)}`}
                    ...
                    <div className="relative h-40 overflow-hidden bg-surface-container-low">
                      ...
                      <button onClick={e => { e.preventDefault(); e.stopPropagation(); toggleFavorite(toSlug(t.title), 'image-gen') }}
                        ...
                        <span className={`material-symbols-outlined ${isFavorite(toSlug(t.title), 'image-gen') ? 'text-red-500' : 'text-text-muted'}`} style={{ fontSize: 18 }}>{isFavorite(toSlug(t.title), 'image-gen') ? 'favorite' : 'favorite_border'}</span>
                      </button>
                    </div>
                    <div className="p-4 flex flex-col flex-1">
                      ...
                      <div className="mt-auto flex items-center justify-between border-t border-border-light pt-3">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); navigate(`/image-gen/${toSlug(t.title)}/preview`) }} className="px-3 py-1.5 border border-primary text-primary rounded hover:bg-primary hover:text-surface transition-colors text-[11px] font-medium">
                            Preview
                          </button>
                        </div>
                      </div>
        ...
        <section className="px-6 py-16 my-6 bg-surface-container-lowest border-y border-border-light">
          <div className="flex flex-col lg:flex-row gap-8">
            <div className="w-full lg:w-1/3 flex flex-col justify-center border border-border-light border-dashed rounded-xl p-8 bg-surface">
              <h2 className="text-[24px] font-semibold text-text-main mb-4">Featured Image Tools</h2>
              <p className="text-[15px] text-text-muted leading-relaxed mb-8">
                Every month, our curators pick the best new image generation tools — from text-to-image to 3D models.
              </p>
            ...
            <div className="w-full lg:w-2/3 grid grid-cols-1 sm:grid-cols-2 gap-6">
              <img
                src="https://picsum.photos/seed/featured-image-gen/800/200"
                alt="Featured Image Tools"
      ...
      <footer className="bg-text-main text-surface w-full py-10 px-6 grid grid-cols-2 md:grid-cols-4 gap-6 border-t border-outline">
        <div className="col-span-2 md:col-span-1 flex flex-col gap-4">
          ...
          <p className="text-[15px] text-secondary-fixed-dim mt-4 leading-relaxed">
            &copy; 2026 AI Agents Marketplace. All rights reserved. 12k+ templates available.
          </p>
        </div>
        <div className="flex flex-col gap-3">
          <h4 className="text-xs font-semibold text-surface font-bold uppercase tracking-wider mb-2">Image Gen</h4>
        ...
        <div className="col-span-2 md:col-span-1 flex flex-col gap-6 justify-end items-start md:items-end mt-8 md:mt-0">
          <div className="text-left md:text-right">
            ...
            <div className="text-[11px] font-medium text-secondary-fixed-dim uppercase tracking-wider">Models Available</div>
          </div>
          <div className="text-left md:text-right">
            <div className="text-[24px] font-semibold text-surface mb-1">14,520</div>
            <div className="text-[11px] font-medium text-secondary-fixed-dim uppercase tracking-wider">Images Generated</div>
          </div>
        </div>
      </footer>
