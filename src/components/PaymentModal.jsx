          ...
          {step === 'pay' && method && (
            <>
              ...
              <div className="overflow-y-auto flex-1 p-6">
                {method.id === 'qris' && (
                  <QRISView total={total} seed={qrSeed} onSuccess={() => { setStep('success'); setTimeout(() => { onSuccess(selected) }, 1500) }} canvasRef={canvasRef} />
                )}
                {['dana', 'gopay', 'shopeepay', 'ovo', 'linkaja'].includes(method.id) && (
                  <EwalletView method={method} total={total} onSuccess={() => { setStep('success'); setTimeout(() => { onSuccess(selected) }, 1500) }} />
                )}
                {['bca', 'mandiri', 'bni', 'bri'].includes(method.id) && (
                  <BankView method={method} total={total} onSuccess={() => { setStep('success'); setTimeout(() => { onSuccess(selected) }, 1500) }} />
                )}
                {['alfamart', 'indomaret'].includes(method.id) && (
                  <StoreView method={method} total={total} onSuccess={() => { setStep('success'); setTimeout(() => { onSuccess(selected) }, 1500) }} />
                )}
              </div>
            </>
          )}
