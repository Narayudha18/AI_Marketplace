  ...
  return (
    <>
      <div className="bg-gradient-to-r from-primary-container to-blue-600 text-on-primary-container px-6 py-2.5 text-center text-xs font-semibold flex justify-center items-center gap-3">
        <span className="bg-white/20 text-white px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider">{t('nav.new')}</span>
        <span>{t('nav.announcement')}</span>
        <Link to="/templates" className="bg-text-main text-surface px-4 py-1.5 rounded text-[11px] font-bold hover:opacity-90 transition-opacity whitespace-nowrap">{t('nav.explore')}</Link>
      </div>
      ...
      <header className="bg-text-main flex flex-col w-full sticky top-0 z-40">
        <div className="px-6 h-14 flex items-center justify-between border-b border-white/5">
          <Link to="/" className="text-lg font-bold text-surface tracking-tight">{t('nav.brand')}</Link>
          ...
          <div className="flex items-center gap-3">
            <Link to="/start-selling" className="hidden sm:flex text-surface-variant hover:text-surface transition-colors text-xs font-semibold">{t('nav.startSelling')}</Link>
            ...
            <button onClick={() => setLang(lang === 'en' ? 'id' : 'en')} className="text-surface-variant hover:text-surface transition-colors cursor-pointer text-[11px] font-bold px-2 py-1 rounded border border-white/10 hover:border-white/30" title={t('lang.name')}>
              {lang === 'en' ? 'ID' : 'EN'}
            </button>
      ...
      {location.pathname === '/' && (
        <div className="bg-surface border-b border-border-light">
          <div className="max-w-[1440px] mx-auto px-6 h-11 flex items-center gap-1 overflow-x-auto">
            {['All Items', 'GPT Agents', 'Voice AI', 'Image Gen', 'RAG Pipelines', 'Workflow', 'Analytics', 'Fine-tuning', 'Deployment', 'Monitoring', 'Security'].map((item, idx) => {
              const label = subLinks[idx] || item
