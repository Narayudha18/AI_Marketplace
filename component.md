...
| Auth Page | **Profile** | `src/pages/Profile.jsx` | — | `activeTab`, `orders[]`, `expandedOrder`, `editMode`, `editForm`, `newAddress`, `showAddressForm`, `passForm`, `passError`, `cropImage`, `toast` | Tab navigation (My Profile, Orders, Addresses, Settings); edit name/email/bio inline; avatar upload via crop modal; password change with validation; add/remove addresses; dark mode toggle; toast notification feedback | `users`, `orders`, `addresses` | `GET /api/auth/me`, `PUT /api/auth/picture`, `GET/PUT /api/user/profile`, `GET/POST/DELETE /api/user/addresses`, `GET /api/user/orders` |
