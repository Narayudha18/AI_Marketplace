...
| Auth Page | **Profile** | `src/pages/Profile.jsx` | — | `activeTab`, `orders[]`, `expandedOrder`, `editForm`, `saving`, `newAddress`, `showAddressForm`, `passForm`, `passError`, `cropImage`, `toast`, `activities[]` | 5-tab standalone dashboard; Overview (stats cards + activity timeline); Profile (avatar upload crop + inline edit name/email/bio); Orders (expandable history); Addresses (add/remove); Settings (password change, dark mode toggle, notification prefs, sign out). All with toast notification feedback. | `users`, `orders`, `addresses`, `activities` | `GET /api/auth/me`, `PUT /api/auth/picture`, `GET/PUT /api/user/profile`, `GET/POST/DELETE /api/user/addresses`, `GET /api/user/orders`, `GET /api/user/activities` |
